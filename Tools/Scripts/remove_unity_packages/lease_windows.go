...
func replacePathAtomically(stage, target string) error {
	...
	result, _, callErr := moveFileExW.Call(uintptr(unsafe.Pointer(stagePointer)), uintptr(unsafe.Pointer(targetPointer)), movefileReplaceExisting|movefileWriteThrough)
	if result == 0 {
...
func syncParentDirectory(string) error {
	// File Sync plus MOVEFILE_WRITE_THROUGH is the Windows durability contract.
	return nil
}
