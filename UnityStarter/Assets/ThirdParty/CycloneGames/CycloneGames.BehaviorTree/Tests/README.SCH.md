...
7. 点击 `Run Production Certification Matrix`，运行带生产预算阈值的重型认证矩阵。
...
- `Production Budgets`
  - 保存平均帧耗时、最大帧耗时、按负载规模估算的托管内存容量、GC 次数和有效 tick 比例的 pass/fail 阈值
...
5. 在声明某个配置可进入生产前，先运行 `Run Production Certification Matrix`。
6. 想观察真实帧表现、设备差异、LOD 收益、联机混合调度成本或长时间 soak 时，用生成的 PlayMode benchmark 场景。
