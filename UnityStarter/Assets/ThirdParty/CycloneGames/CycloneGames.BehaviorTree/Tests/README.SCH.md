...
本目录提供 `CycloneGames.BehaviorTree` 的 EditMode/PlayMode 测试、benchmark runner、调度 profile、soak 采样和结果导出。
...
Editor 测试覆盖成功路径和失败路径。远端 payload 测试验证畸形输入会在本地 runtime state 改变前被拒绝。
...
7. 点击 `Run Production Certification Matrix`，运行工具中以该名称保存的预算矩阵。
8. 点击 `Create PlayMode Benchmark Scene`，按当前配置生成 PlayMode benchmark 场景。
9. 点击 `Create Scene From Preset`，按当前预设和复杂度生成场景。
10. 点击 `Create Scale Matrix Scene`，生成一个会自动顺序执行当前复杂度全部数量级预设的场景。
11. 点击 `Create Full Matrix Scene`，生成一个会自动顺序执行完整矩阵的场景。
12. 点击 `Create PriorityManaged Comparison Scene`，生成一个会自动运行调度对照批次的场景。
13. 点击 `Create Production Certification Scene`，生成一个会自动运行认证矩阵的 PlayMode 场景。
14. 进入 PlayMode，runner 会自动开始执行。
...
- `Production Budgets`
  - 保存平均帧耗时、最大帧耗时、按负载规模估算的托管内存容量、GC 次数和有效 tick 比例的 pass/fail 阈值；通过这些阈值不等于 Player 或平台认证
  - `Max Memory Delta` 是一次 benchmark 会话级别的驻留内存容量预算，不是每帧分配预算；热路径分配稳定性应结合 GC 次数、delta flush 分配守卫和 soak 内存漂移一起判断
...
- `Create Production Certification Scene` 生成认证矩阵 runner，会顺序执行 heavy stress、network-mixed 和 soak 等关键场景。
...
4. 点击 `Export Last Result to Default Folder` 可以同时把 CSV 和 JSON 写入 `Application.persistentDataPath/BehaviorTreeBenchmarkResults`。

当矩阵跑完后：

1. 点击 `Export Last Matrix as CSV` 或 `Export Last Matrix as JSON`。
2. 每一行或每个 JSON 项对应一个数量级、复杂度、调度策略组合的结果。
3. 点击 `Export Last Matrix to Default Folder` 可以同时把 CSV 和 JSON 写入默认 benchmark 结果目录。

对于 PlayMode 自动生成的场景，导出是自动完成的，runner 会把最终导出路径打印到 Unity Console。

测试和 Editor 工具共用 `BehaviorTreeBenchmarkExportUtility` 进行序列化。
...
- `ProductionBudgetPassed`
  - 只有平均帧耗时、最大帧耗时、托管内存增量、GC 和有效 tick 比例全部通过预算时才为 true
- `BudgetSummary`
  - 面向人的本次运行 pass/fail 摘要
...
5. 使用 `Run Production Certification Matrix` 评估配置预算，再在代表性硬件的 release Player build 中重复该负载。
6. 使用生成的 PlayMode benchmark 场景检查帧行为、LOD 调度权衡和长时间 soak，再进行目标设备验证。
7. 把每次结果导出成 CSV / JSON，方便做历史对比、不同设备档位对比和版本回归分析。
