...
Core assembly 遵循现有 RPGFoundation networking 包风格：位于 `Core/` 目录，assembly name 以 `.Networking.Core` 结尾，root namespace 仍保持 `CycloneGames.RPGFoundation.Projectile.Networking`。它引用 `CycloneGames.RPGFoundation.Projectile.Core` 和 `CycloneGames.Networking.Core`，但不引用 Unity package、后端 SDK、Unity 场景对象、PlayerSettings scripting define symbol、具体 transport 或 DI 容器。

## 与 CycloneGames.Networking 协同

这个包的定位是和 `CycloneGames.Networking` 协同，而不是替代它。
...
本包不会自己发送网络包。项目侧 networking adapter 应负责注册协议、通过选定 transport 序列化消息、校验入站消息，并把已接受的消息转发给 gameplay 系统。

## 协议

`ProjectileNetworkProtocol` 在 Cyclone module range 内拥有 `17000-17999` message id。
...
Snapshot application 故意交给项目代码处理。视觉插值、预测回滚、仅表现 projectile 和服务器持有的 projectile world 往往需要不同的所有权策略。
...
当无法使用 Unity Editor 自动化时，应使用当前分支引用编译纯 C# assembly，并确认 `CycloneGames.RPGFoundation.Projectile.Networking.Core` 没有泄漏 `UnityEngine` 引用。
