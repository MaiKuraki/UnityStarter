...
- [CycloneGames.GameplayFramework](#cyclonegamesgameplayframework)
  ...
  - [可选 Networking 集成](#可选-networking-集成)
  - [快速开始](#快速开始)
    - [前提条件](#前提条件)
    - [最小配置](#最小配置)
      - [1. 创建必需预制体](#1-创建必需预制体)
...
基础 `CycloneGames.GameplayFramework.Runtime` 程序集不要求 `CycloneGames.Networking`。网络支持被隔离在 `Runtime/Scripts/Integrations/Networking`，并由 `CycloneGames.GameplayFramework.Runtime.Integrations.Networking` 编译。

只有引入 `CycloneGames.Networking` 的项目才需要使用这个 integration。

### NetworkGameSessionAdapter

`NetworkGameSessionAdapter` 继承 `GameSession`，并把 session 规则桥接到 Cyclone networking。它保存 `INetworkManager`，绑定 `PlayerController` 与 `INetConnection`，在登录审批时检查认证连接，并通过 `INetworkManager.DisconnectClient` 断开玩家。
...
`GameplayNetworkAuthorityRole` 描述 actor 是 `ServerAuthority`、`AutonomousProxy`、`SimulatedProxy` 还是 `None`。标准服务端权威项目可以使用 `ServerAuthoritativeGameplayAuthorityResolver`。

`GameplayReplicationPolicy` 描述 visibility、channel、distance、tick interval、priority、layer mask、是否包含 owner 和是否要求认证连接。常用 preset 包括 `OwnerReliable`、`TeamReliable`、`AlwaysRelevantReliable` 和 `AreaUnreliable(distance)`。

`NetworkedGameplayActor` 是 `Actor` 的网络边界数据，包含 network id、owner connection、owner player id、team id、interest layer、relevance flag 和 interest position。`GameplayNetworkObserverRegistry` 与 `GameplayNetworkObserverResolver` 用于构建 area、team、owner-only 和 always-relevant 复制的 observer set。

这个 integration 也适合为 `CycloneGames.GameplayAbilities.Networking` 提供 observer list，同时 Ability 复制本身仍保持在 GAS networking 包中。
