...
| 字段 | 必需 | 含义 |
| --- | --- | --- |
| `schemaVersion` | 是 | Runtime format version。新文件使用 `InputConfiguration.CurrentSchemaVersion` 暴露的当前值。 |
...
Player ID 是配置 identity，不是 list position。双人产品通常使用 `0` 与 `1`；只要处于配置 limit 内，也可以使用非连续 ID。

## Control scheme

Control scheme 描述哪些设备可以满足一个 player slot：
...
不需要额外约束时应保持为空。空的 advanced field 是正常配置，可减少配置耦合。

Interaction 配置在 action binding 上。Composite part 可以定义 part-local processor；part-local interaction 必须为空。

## Direct path 与 picker

Device-binding row 包含可输入任意有效 Input System control path 的 text field，以及选择已知项目常量的紧凑 picker。示例：
...
Picker 是 authoring 快捷入口，不代表 Input System 接受的全部 path。
...
只有 `StartListeningForPlayers` active 时才使用 join action。

- Root join path 是 shared candidate。
- Slot join path 是 slot-specific candidate。
- Device-locking mode 将触发设备分配给 joined player。
- Shared-device mode 允许多个 declared player 使用同一物理设备。

同一个 join action 内不要重复 direct path。Lobby policy 可以让 root join action 与 slot join action 有意共享 control。
