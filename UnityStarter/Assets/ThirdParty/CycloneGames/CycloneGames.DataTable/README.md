...
Runtime manifest validation intentionally keeps a tiny SHA-256 helper inside `CycloneGames.DataTable.Core` instead of depending on the IO package. Build and editor tooling can use `CycloneGames.IO.FileHasher`, `CycloneGames.IO.ContentHasher`, or `XxHash64` to generate manifest values before they are passed into DataTable.
