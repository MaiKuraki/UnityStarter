...
{
    public sealed class AssetManagementDataTableRawFileBytesLoader : IDataTableBytesProvider, IDisposable
    {
        ...
        private readonly List<IRawFileHandle> _handles;
            ...
            DataTableManifest manifest = null)
        {
            ...
            _handles = new List<IRawFileHandle>(Math.Max(0, initialCapacity));
            _manifest = manifest;
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (handle == null || !string.IsNullOrEmpty(handle.Error))
            {
                byte[] fallbackBytes = TryLoadEditorFile(location);
                if (fallbackBytes != null)
                {
                ...
                string error = handle == null ? "Raw file handle is null." : handle.Error;
                throw new InvalidOperationException(
                    $"Failed to load data table raw file. Table={normalizedName}, Location={location}, Error={error}");
            }
            ...
            {
                throw new InvalidOperationException(
                    $"Loaded data table raw file is empty. Table={normalizedName}, Location={location}");
            }
