...
本模块提供两个静态工具类，并集成来自 `CycloneGames.Hash.Core` 的共享哈希基础能力：

| 条目              | 用途                                                    |
| ----------------- | ------------------------------------------------------- |
| `FileUtility`     | 哈希计算、文件/流比较、带校验的智能文件复制，零 GC 设计     |
| `FilePathUtility` | 为 `UnityWebRequest` 生成跨平台正确的 URI                   |
| `XxHash64`        | 由 `CycloneGames.Hash.Core` 提供的共享非密码学哈希基础类型 |
...
- [文件工具库](#文件工具库)
  ...
  - [游戏开发应用场景](#游戏开发应用场景)
    - [热更新 / 资源补丁](#热更新--资源补丁)
    - [AssetBundle 完整性校验](#assetbundle-完整性校验)
    - [存档系统](#存档系统)
    - [构建管线 / 编辑器工具](#构建管线--编辑器工具)
    - [快速本地变更检测](#快速本地变更检测)
    - [直接使用 XxHash64](#直接使用-xxhash64)
    - [跨平台资源加载](#跨平台资源加载)
...
`FileUtility` 在 `HashAlgorithmType.XxHash64` 路径中使用共享的 `CycloneGames.Hash.Core.XxHash64` 实现。算法归属于 `CycloneGames.Hash` 模块；Utility 模块只负责围绕文件和流的读取、缓冲、比较与复制流程。
...
xxHash64 在原生代码中比 MD5 快约 30 倍。在当前 `CycloneGames.Hash.Core` 的纯 C# 实现（无 SIMD）中，XxHash64 实测吞吐约 ~160–190 MB/s —— 与原生 MD5（~244 MB/s，经由 Windows CNG）相当，比 SHA256（~17 MB/s）快约 10 倍。更重要的是，`struct` 设计彻底消除了所有托管分配开销，使其成为纯内存和同步操作的最优选择。
...
| 依赖                             | 用途                                                                                        |
| -------------------------------- | ------------------------------------------------------------------------------------------- |
| `CycloneGames.Hash.Core`         | 共享的 `XxHash64`、`Fnv1a64` 和稳定哈希基础能力                                             |
...
无需外部第三方包。`CycloneGames.Hash.Core` 是本地 CycloneGames 包，全平台可用（包括 WebGL）。
