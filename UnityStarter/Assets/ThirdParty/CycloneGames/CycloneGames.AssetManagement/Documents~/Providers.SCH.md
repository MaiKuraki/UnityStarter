...
模块把每个 provider 作为 `IAssetModule` 暴露，各有不同的能力面。Addressables 与 YooAsset 不能同时通过这些 adapter 激活 —— 框架的 AssetBundle runtime guard 建立单一 provider authority，直到共存、shutdown 顺序与内存行为作为完整产品配置得到验证。
...
可选源只在以下全部成立时编译：依赖已安装到 `UnityStarter/Packages/manifest.json` 并在 `packages-lock.json` 中解析、版本满足 provider asmdef 范围、composition asmdef 引用 provider/integration assembly（`autoReferenced: false`）、assembly-local 条件满足。
...
Addressables 与 YooAsset 只暴露异步 `IAssetSceneLoader` 能力。Unity scene 集成包含帧边界的异步生命周期工作，因此 provider-neutral API 不宣传同帧完成。高级重载携带 Unity `LoadSceneParameters`，支持 `LocalPhysicsMode.None`、`Physics2D`、`Physics3D` 与合法的 `Physics2D | Physics3D` 组合。加载的 scene 持有这些 local physics world，卸载时销毁。
...
YooAsset provider 目标为 `com.tuyoogame.yooasset` `[3.0.4,3.0.5)`。`YooAssetModule` 独占进程全局 Yoo runtime。
...
| 现象 | 可能原因 | 解决方法 |
| --- | --- | --- |
| Provider assembly 不编译 | 依赖缺失或版本超出范围 | 安装并锁定确切 SDK 版本；确认 `versionDefines` 并从 composition asmdef 引用 assembly |
