...
namespace CycloneGames.Localization.Editor
{
    public sealed class AssetTableEditor : EditorWindow
    {
        ...
        private const float MetaRowH   = 80f;
        ...
        private void DiscoverTables()
        {
            ...
            var guids = AssetDatabase.FindAssets("t:AssetTable");
            var idSet = new SortedSet<string>(StringComparer.Ordinal);
            foreach (var g in guids)
            {
                var t = AssetDatabase.LoadAssetAtPath<AssetTable>(AssetDatabase.GUIDToAssetPath(g));
        ...
        private void RefreshColumnsForTable(string tableId)
        {
            ...
            _keysDirty = true;
            _expandedKeys.Clear();
            foreach (var g in AssetDatabase.FindAssets("t:AssetTable"))
            {
        ...
        private void RefreshMetadata(string tableId)
        {
            ...
            foreach (var g in AssetDatabase.FindAssets("t:StringTableMetadata"))
            {
                ...
                if (m != null && m.TableId == tableId && m.TableType == TableType.Asset)
                {
                    _metadata = m;
        ...
        private void DrawTableArea()
        {
            ...
            for (int v = 0; v < _visibleKeyIndices.Count; v++)
            {
                ...
                for (int c = 0; c < _columns.Count; c++)
                {
                    ...
                    {
                        DrawAssetRefField(col.EntriesProp.GetArrayElementAtIndex(idx).FindPropertyRelative("Asset"), cellR);
                    }
                    else
                    {
        ...
        private void DrawAssetRefField(SerializedProperty assetRefProp, Rect cellR)
        {
            var guidProp = assetRefProp.FindPropertyRelative("m_GUID");
            var locProp = assetRefProp.FindPropertyRelative("m_Location");
            ...
            EditorGUI.BeginChangeCheck();
            var newObj = EditorGUI.ObjectField(cellR, obj, typeof(UnityEngine.Object), false);
        ...
        private void DrawMetadataSubRow(string key, float x, float y, float w)
        {
            ...
            Rect taR = new(cx, cy + 14, cw, 28);
            commentProp.stringValue = EditorGUI.TextArea(taR, commentProp.stringValue);

            float row2Y = cy + 46;
            var lockedProp = entry.FindPropertyRelative("Locked");
            lockedProp.boolValue = EditorGUI.ToggleLeft(new Rect(cx, row2Y, 60, 16), "Locked", lockedProp.boolValue);

            GUI.Label(new Rect(cx + 68, row2Y, 30, 16), "Tags", MetaLbl);
            var tagsProp = entry.FindPropertyRelative("Tags");
            tagsProp.stringValue = EditorGUI.TextField(new Rect(cx + 100, row2Y, cw - 100, 16), tagsProp.stringValue);
        ...
        private void CreateMetadataAsset()
        {
            ...
            string assetPath = Path.Combine(dir, $"StringTableMetadata_{tableId}_Asset.asset").Replace('\\', '/');
            if (AssetDatabase.LoadAssetAtPath<StringTableMetadata>(assetPath) != null) { RefreshMetadata(tableId); return; }
            var meta = CreateInstance<StringTableMetadata>();
            var so = new SerializedObject(meta);
            so.FindProperty("tableId").stringValue = tableId;
            so.FindProperty("tableType").enumValueIndex = (int)TableType.Asset;
        ...
        private void CreateNewLocaleTable(string localeCode)
        {
            ...
            string dir = "Assets";
            if (_columns.Count > 0) { string p = AssetDatabase.GetAssetPath(_columns[0].Table); if (!string.IsNullOrEmpty(p)) dir = Path.GetDirectoryName(p); }
            string assetPath = Path.Combine(dir, $"AssetTable_{tableId}_{localeCode}.asset").Replace('\\', '/');
            if (AssetDatabase.LoadAssetAtPath<AssetTable>(assetPath) != null) { Debug.LogWarning($"[Localization] Already exists: {assetPath}"); return; }

            var newTable = CreateInstance<AssetTable>();
