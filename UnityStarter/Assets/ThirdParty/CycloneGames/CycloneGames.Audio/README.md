...
Add an `AudioManager` component to your runtime scene. Assign mixer or profile assets as needed. The manager resolves the authoritative `AudioListener` in this order: the serialized `AudioListener` reference on the component, an existing listener on the main camera, a listener created on the main camera, and finally one on the manager itself. When the listener lives on another object, register it explicitly with `AudioManager.SetAudioListener(listener)` — no scene-wide scan runs at runtime.
...
Static audio APIs self-create the manager on first use. A scene-placed manager is adopted through its own `Awake`, which also destroys duplicate instances; because component `Awake` ordering is undefined, call audio APIs from `Start` or later, or register explicitly with `AudioManager.SetInstance`.

Assign the four optional configuration overrides on `AudioManager` when the product owns their location. Automatic discovery is a synchronous main-thread, Editor-only compatibility fallback: in the Editor it uses `AssetDatabase.FindAssets` by type (first found wins, with a warning when several assets exist), while Player builds perform no scan and no load — discovery returns null and the negative cache holds until `ClearCache`, subsystem registration, or destruction of the cached Unity object allows a new search. A discovery exception does not publish a cached miss, so the next request may retry. Production builds must wire configuration explicitly: assign the serialized overrides, or load the assets through your asset management system (for example CycloneGames.AssetManagement) and call the corresponding `SetConfig` method before the first configuration request.
...
`AudioEventRouter` admits at most one active looping task per trigger index. Repeated `StartLoopingTrigger` calls for the same index and trigger object are idempotent until that loop exits or the router cancels its loops; callers that intentionally need overlapping loops must use distinct trigger indices. Replacing the trigger object at an index and starting it immediately cancels that slot's prior worker. Disable or destruction invalidates the complete loop generation. When the trigger-array length changes, the next start request invalidates the old generation; without a new start request, an already running loop exits at its next post-delay index/reference check. A late completion cannot clear a newly started loop. Trigger wiring is exposed through the `AudioEventRouter.Triggers` property (serialized name `triggers`, unchanged); element mutation is allowed because the array is shared with the serializer.
