...
{
    /// <summary>
    /// NativeContainer-backed spatial-grid interest manager using sort-based spatial hashing.
    /// Alternative to <see cref="GridInterestManager"/> with reusable native storage,
    /// sorted entity data, and <c>math.distancesq</c> distance checks.
    ///
    /// This assembly does not currently schedule Jobs or reference Burst. Select this
    /// implementation only after benchmarking the real entity and observer distribution
    /// on the target platform.
    ///
    /// Architecture:
    ///   PreUpdate:  Marshal entities → compute cell keys → sort → build cell range index
    ...
    /// </summary>
    public sealed class NativeGridInterestManager : IInterestManager, IDisposable
    {
        ...
        // Native data reused across frames; growth can allocate native memory.
        ...
        public NativeGridInterestManager(float cellSize = 50f, float visibilityRange = 100f, int initialCapacity = 1024)
        {
            _cellSize = cellSize;
        ...
        public void PreUpdate(IReadOnlyList<INetworkEntity> allEntities)
        {
            ...
            // 2. Sort by cell key via NativeSortExtension.
            if (count > 1)
