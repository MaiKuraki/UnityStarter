...
| Assembly | 职责 |
| --- | --- |
| `CycloneGames.Networking.Core` | 引擎无关 networking contract、profile、catalog、replication、session、security、validation helper、deterministic math payload 与 hashing support。 |
| `CycloneGames.Networking.Unity.Runtime` | Unity runtime bridge 和 adapter-facing helper。 |
| `CycloneGames.Networking.DOD.Runtime` | 面向 Burst/Jobs 的 runtime helper，桥接 Core、Unity.Runtime、Burst、Collections、Mathematics 和 Jobs。 |
...
`CycloneGames.Networking.Core` 是 `noEngineReferences: true`，public contract 不暴露 UnityEngine 类型。它直接引用 `CycloneGames.DeterministicMath.Core` 和 `CycloneGames.Hash.Core`；这两个是核心依赖，不是可选 integration。Unity 相关行为隔离在 runtime、adapter、serializer、DOD 或 editor assembly 中。
...
| `INetworkCryptoProvider` | Payload protection abstraction。内置 provider 是 no-op implementation。 |
...
- `HmacSha256NetworkMessageSigner` 已存在，但 `NetworkSecurityPresets` 不会自动创建它；当 policy 要求签名时，需要 composition root 显式传入已启用的 signer。
- `NoopNetworkCryptoProvider` 是当前唯一内置 `INetworkCryptoProvider`。当 backend 支持时，优先使用 TLS、DTLS 或 WSS 等 transport-level confidentiality。只有产品威胁模型确实需要，并且已在所有目标平台验证后，才添加 application-layer crypto。
- `NetworkAuthenticationProviderChain` 与 `RejectAllNetworkAuthenticationProvider` 已存在。JWT、Steam、platform identity 或 backend session 等正式身份提供者属于项目或 adapter 职责。
- `NoopNetworkAntiCheatSignalSink` 与 `RecordingNetworkAntiCheatSignalSink` 已存在。生产 telemetry、moderation、disconnect policy 和 analytics sink 未内置。
- `NetworkSecurityAudit` 会评估 release encryption、signer availability、rate limiting、replay protection、anti-cheat sink wiring 和 default authentication policy。CI 或 batchmode audit entry point 仍属于项目自有流程。
- `NetworkSecurityPresets.CreateProductionServerSecurityBaseline` 是严格服务器 helper。它会在构造阶段拒绝 disabled signer、no-op anti-cheat sink 和缺失的 rate limiter，然后在 default policy 上启用 authenticated connection、encrypted-transport policy、replay protection、message signature 和 rate limiting。
...
上例中的 `signer`、`antiCheatSink` 和 `rateLimiter` 参数必须是项目自有 runtime object；对于 `HmacSha256NetworkMessageSigner` 这类 `IDisposable` provider，由 composition root 负责释放。Key ownership、long-term credential storage、moderation workflow 和 platform identity verification 位于本包外部。
...
Serializer integration assembly 位于 `Unity.Runtime/Serializers/`，并直接引用 `CycloneGames.Networking.Core`。
...
`MessagePackSerializerAdapter` 在序列化时使用 thread-local pooled `IBufferWriter<byte>`，在 `ReadOnlySpan<byte>` 反序列化时使用基于 `ArrayPool<byte>` 的拷贝。这样移除了 adapter 自身的 per-call `ArrayBufferWriter<byte>` 和 `ToArray()` 分配，但最终分配行为仍取决于 MessagePack-CSharp formatter，并且必须在 IL2CPP target 上验证。
