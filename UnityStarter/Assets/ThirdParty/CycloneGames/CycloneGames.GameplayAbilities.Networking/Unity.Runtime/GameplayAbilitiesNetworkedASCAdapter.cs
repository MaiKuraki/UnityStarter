...
namespace CycloneGames.GameplayAbilities.Networking
{
    ...
    public sealed class GameplayAbilitiesNetworkedASCAdapter : INetworkedASC, INetworkedASCConnectionScopedFullState, IDisposable
    {
        ...
        private void AssertRuntimeThread()
        {
            ...
            GASNetLogger.LogWarning($"[GAS Integration] Adapter accessed from thread {currentThreadId}; runtime thread is {_runtimeThreadId}.");
        }
        ...
        public void OnReplicatedEffectApplied(EffectReplicationData data)
        {
            ...
            {
                GASNetLogger.LogWarning($"[GAS Integration] Unknown effectDefinitionId={data.EffectDefinitionId}.");
                return;
            }
        ...
        public void OnReplicatedEffectRemoved(int effectInstanceId)
        {
            ...
            GASNetLogger.LogWarning($"[GAS Integration] Effect remove not applied for remote instance {effectInstanceId}. " +
        ...
        public void OnReplicatedStackChanged(int effectInstanceId, int newStackCount)
        {
            ...
            GASNetLogger.LogWarning($"[GAS Integration] Stack change not applied for remote instance {effectInstanceId}. " +
        ...
        public void OnReplicatedEffectUpdated(EffectUpdateData data)
        {
            ...
            GASNetLogger.LogWarning($"[GAS Integration] Effect update not applied for remote instance {data.EffectInstanceId}. " +
        ...
        public GASFullStateData CaptureFullState()
        {
            ...
            return new GASFullStateData
            {
                ...
                StateChecksum = ComputeNetworkStateChecksum(
                    _grantedAbilitiesBuffer,
                    abilityCount,
                    _currentEffectsScratch,
                    effectCount,
                    _attributeEntriesBuffer,
                    attributeCount,
                    _fullStateTagHashesBuffer,
                    tagCount),
        ...
        public void OnFullState(GASFullStateData data)
        {
            ...
            {
                GASNetLogger.LogWarning($"[GAS Integration] Ignored full state for target {data.TargetNetworkId}; adapter target is {NetworkId}.");
                return;
            }
        ...
        private bool AcceptTargetNetworkId(uint targetNetworkId, string operation)
        {
            ...
            GASNetLogger.LogWarning($"[GAS Integration] Ignored {operation} for target {targetNetworkId}; adapter target is {NetworkId}.");
            return false;
        }
        ...
        public ReplicatedAbilitySystemStateDelta CapturePendingReplicatedStateDelta()
        {
            ...
            if (_stateDeltaBuffer.HasChanges)
            {
                ...
                _stateDeltaBuffer.StateChecksum = ComputeCurrentNetworkStateChecksum();
            }
        ...
        private SetByCallerEntry[] BuildSetByCallerEntries(GameplayEffectSpec spec, int effectIndex, out int copiedCount)
        {
            ...
            Array.Sort(networkEntries, 0, copiedCount, SetByCallerEntryComparer.Instance);
        ...
        private static bool IsStackOnlyChange(EffectReplicationData previous, EffectReplicationData current)
        {
            return previous.StackCount != current.StackCount
                ...
                && previous.DurationRaw == current.DurationRaw
                && previous.TimeRemainingRaw == current.TimeRemainingRaw
                && previous.PeriodTimeRemainingRaw == current.PeriodTimeRemainingRaw
            ...
            int rightCount)
        {
            ...
            for (int i = 0; i < leftCount; i++)
            {
                ...
                if (leftEntries[i].ValueRaw != rightEntries[i].ValueRaw)
                    return false;
            }
        ...
        private static EffectUpdateData ToEffectUpdateData(EffectReplicationData data)
        {
            return new EffectUpdateData
            {
                ...
                DurationRaw = data.DurationRaw,
                TimeRemainingRaw = data.TimeRemainingRaw,
                PeriodTimeRemainingRaw = data.PeriodTimeRemainingRaw,
        ...
        private uint ComputeCurrentNetworkStateChecksum()
        {
            ...
            int attributeCount = FillAttributeEntriesFromAsc(ref _checksumAttributeEntriesBuffer);
            int tagCount = FillTagHashesFromAsc(ref _checksumTagHashesBuffer);

            return ComputeNetworkStateChecksum(
                _checksumGrantedAbilitiesBuffer,
                abilityCount,
                _checksumEffectsBuffer,
                effectCount,
                _checksumAttributeEntriesBuffer,
                attributeCount,
                _checksumTagHashesBuffer,
                tagCount);
        }
        ...
        private static uint ComputeNetworkStateChecksum(
            ...
            int tagCount)
        {
            return GASNetworkStateChecksum.Compute(
                abilities,
                abilityCount,
                effects,
                effectCount,
        ...
        private sealed class SetByCallerEntryComparer : IComparer<SetByCallerEntry>
        {
            public static readonly SetByCallerEntryComparer Instance = new SetByCallerEntryComparer();

            public int Compare(SetByCallerEntry x, SetByCallerEntry y)
            {
                return x.TagHash.CompareTo(y.TagHash);
            }
        }
    }
}
