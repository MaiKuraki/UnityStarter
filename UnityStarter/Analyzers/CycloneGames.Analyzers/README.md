...
Within repository-owned source, the `GameObject.Find` and scene-wide find family (`CG0010`, `CG0011`) is scoped to production runtime code. Sources below an `Editor/`, `Tests/`, `Samples/`, or `Sample/` path segment are Editor tooling, validators, debug windows, or tests whose on-demand scene scans are legitimate, so the family stays silent there. The classification is cached per syntax tree, uses the same host-filesystem path-segment comparison as ownership, and the empty Roslyn test path stays classified as production runtime so focused hosts remain governed. The classification lives in `AnalyzerSourceScope` and is shared with the `UnityEditor` usage rule (`CG0042`).

`<unity-project>/Assets/Default.ruleset` is the committed Unity enforcement policy. `CG0010` remains an error. `CG0011` and `CG0013` remain visible as warnings while existing scene-discovery and timer call sites are migrated; this prevents analyzer activation from converting known migration debt into an unrelated compile outage. The GameplayFramework camera-output discovery (`CinemachineCameraOutput`) no longer uses the scene-wide find APIs: its bind-time, authoring-opt-in resolution enumerates the output's own Scene through root objects, so GameplayFramework runtime code contributes no `CG0011` call sites. Remaining `CG0011` debt sits in modules outside GameplayFramework and is owned by those modules. The two name-based target lookups in the GameplayAbilities sample are explicit, locally documented `CG0010` exceptions for that sample scene only. Their production replacement boundary is a project-owned targeting service.
...
| CG0011 | scene-wide `FindObjectOfType` APIs in production code | Error |
